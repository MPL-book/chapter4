...
phmc_mpl = function(ph.formula,pi.formula,data,control,...){
  ...
  for(iter in 1:control$maxIter[1]){
    ...
    for(k in 1:control$maxIter[2]){
      ...
      #check for cured fraction
      if(any(c(pi_Z_o, pi_Z_r, pi_Z_l, pi_Z_i) == 1)){
        stop("The estimate of the uncured fraction has gone to 1. There may not be a cured fraction in your data.
